...
import os
import torch
from transformer6 import get_model
import time
...
def preprocess(input_file, y_date, seq_len=16, batch_size=32, exclude_date=None, d_model=35):
    ...
    """
    #타임스탬프를 from 2015–01–28 to 2016–05–28. 17개
    ...
    for i, row in df.iterrows():
        ...
        #print(exclude_date)
        if row['timestamp'] in exclude_date:
            pass
        user = row['userId']
        ...
        items = list(row.values)[2:]  # items are one-hot encoded (22)
        #print(items)
        ...
        #print("year", year) #1
        #print("month", month) #12
        #print("segmentation_array:", segmentation_array) #4
        #print("value_features:", value_features) #3
        #print("items:", items)   #22
        #print("data:", data)
        #print(y_date)
        if row['timestamp'] == y_date and user in x_users.keys():
            #print(y_date)
    ...
    i=0
    for user in users:
        #print("i :", i, np.array(x_users[user]).shape[0])
        #print("i :", i, np.array(x_users[user]).shape)
        #print(user)
    ...
    #print(num_users - num_users % batch_size)
    #print(x_data[:0])

    #나중에 데이터늘리고나서, 아래 주석 풀것!!!!!
    ...
    with torch.no_grad():
        for batch, labels in tqdm(generator):
            ...
            if len(real_recommendations) > 0:
                ...
                continue
            #print("recomm_real : ", real_recommendations)
...
def train_pipeline(args):
    ...
    if not args.no_load_data and os.path.exists(os.path.join(args.data_path, 'data6.npz')):  # modi
        data = np.load(os.path.join(args.data_path, 'data6.npz'))  # modi
        x_train = data['x_train']
        x_test = data['x_test']
        y_train = data['y_train']
        y_test = data['y_test']
        print("data loaded from", os.path.join(args.data_path, 'data6.npz')) # modi
    ...
    for i in range(x_test.shape[0]):
        owned_items.append(x_test[i][-1][-57:]) # modi

    if args.save_data:
        np.savez(os.path.join(args.data_path, 'data6.npz'),  # modi
                 x_train=x_train, y_train=y_train, x_test=x_test, y_test=y_test)
        print("data saved at", os.path.join(args.data_path, 'data6.npz'))  # modi
    ...
    if args.save_weights > 0:
        torch.save(model.state_dict(), os.path.join(args.weights_path, "weights6.pth")) # modi
        print("model saved at", os.path.join(args.weights_path, "weights6.pth"))  # modi


def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataset', type=str, default="data/prep_rating8.csv")   # modi
    parser.add_argument('--batch_size', type=int, default=256) # modi
    parser.add_argument('--seq_len', type=int, default=16)
    parser.add_argument('--n_items', type=int, default=57,    # modi item갯수도 고쳐야함. d_model값 35 - 13(년월)
    ...
    parser.add_argument('--lr', type=float, default=0.001) # modi
    parser.add_argument('--d_model', type=int, default=70, # modi 아이템:9068,년월:13 (userId, timestamp 2개컬럼 뺄것) => 그럼 컬럼갯수 9079
                        help='dimension of the model')
    parser.add_argument('--heads', type=int, default=2,
                        help='number of Transformer heads')  # modi
    parser.add_argument('--dropout', type=float, default=0.2) # modi
    parser.add_argument('--n_layers', type=int, default=2,
                        help='number of Transformer layer')  # modi
    ...
    parser.add_argument('--hidden_size', type=int, default=2048,
                        help='hidden size of the encoders forward layer') # modi
