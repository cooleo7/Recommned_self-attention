...
import os
import torch
from transformer4 import get_model
import time
...
def train_pipeline(args):
    ...
    if not args.no_load_data and os.path.exists(os.path.join(args.data_path, 'data4.npz')):  # modi
        data = np.load(os.path.join(args.data_path, 'data4.npz'))  # modi
        x_train = data['x_train']
        x_test = data['x_test']
        y_train = data['y_train']
        y_test = data['y_test']
        print("data loaded from", os.path.join(args.data_path, 'data4.npz')) # modi
    ...
    if args.save_data:
        np.savez(os.path.join(args.data_path, 'data4.npz'),  # modi
                 x_train=x_train, y_train=y_train, x_test=x_test, y_test=y_test)
        print("data saved at", os.path.join(args.data_path, 'data4.npz'))  # modi
    ...
    if args.save_weights > 0:
        torch.save(model.state_dict(), os.path.join(args.weights_path, "weights4.pth")) # modi
        print("model saved at", os.path.join(args.weights_path, "weights4.pth"))  # modi
...
def get_args():
    ...
    parser.add_argument('--n_items', type=int, default=15,    # modi item갯수도 고쳐야함. d_model값 35 - 13(년월)
    ...
    parser.add_argument('--d_model', type=int, default=28, #modi 아이템:9068,년월:13 (userId, timestamp 2개컬럼 뺄것) => 그럼 컬럼갯수 9079
