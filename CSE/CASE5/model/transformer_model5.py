...
import os
import torch
from transformer5 import get_model
import time
...
def train_pipeline(args):
    ...
    if not args.no_load_data and os.path.exists(os.path.join(args.data_path, 'data5.npz')):  # modi
        data = np.load(os.path.join(args.data_path, 'data5.npz'))  # modi
        x_train = data['x_train']
        x_test = data['x_test']
        y_train = data['y_train']
        y_test = data['y_test']
        print("data loaded from", os.path.join(args.data_path, 'data5.npz')) # modi
    ...
    if args.save_data:
        np.savez(os.path.join(args.data_path, 'data5.npz'),  # modi
                 x_train=x_train, y_train=y_train, x_test=x_test, y_test=y_test)
        print("data saved at", os.path.join(args.data_path, 'data5.npz'))  # modi
    ...
    if args.save_weights > 0:
        torch.save(model.state_dict(), os.path.join(args.weights_path, "weights5.pth")) # modi
        print("model saved at", os.path.join(args.weights_path, "weights5.pth"))  # modi


def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataset', type=str, default="data/prep_rating5.csv")   # modi
